...
{   
    ...
    if(obj.name!="" && obj.lng!="" && obj.genres==undefined && func== false)
    {
        func=true;
...
function name(movie,rest,obj,id)
{
    ...
    {
        if((restNames[i].toLowerCase()).includes(obj.name.toLowerCase())) //check all letters
        {
    ...
    if(movie.length!=0) // check if file NewMovies not empty
    {
        let movieNames= movie.movies.map(x=> x.name);

        for(i=0;i<movieNames.length;i++)
        {
            if((movieNames[i].toLowerCase()).includes(obj.name.toLowerCase())) //check all letters
            {   
...
function genre(movie,rest,obj,id)
{
    ...
    if(Array.isArray(obj.genres))// if there is more than one genre
    {
