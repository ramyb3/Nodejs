...
const genre = async function(obj, data)
{
    ...
    for(k=0;k<obj.length;k++)
    {
        ...
        if(Array.isArray(obj[k]))// if there is more than one genre
        {
        ...
        for(j=0;j<data.length;j++)
        {
            ...
            if(id[i].length>1 && id[i].includes(dataId[j]) && i==j)
            {
                ...
                {   
                    if(id[i][k]!=dataId[j])
                    {
                        if(id[i][k]<21) // get movies from REST API
                        {
                            temp.push(rest.find(x=>x.id==id[i][k]));
                        }

                        if(id[i][k]>20) // get movies from NewMovies file
                        {
            ...
            {   
                names.push({id: id[i][0], data: "There isn't another movie with the same genre!"});  
            }
        }
    }
